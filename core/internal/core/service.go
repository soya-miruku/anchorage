...
func (s *Service) Handle(ctx context.Context, method string, raw json.RawMessage, emit EventEmitter) (any, error) {
	...
	case "volumes.backup":
		...
		var params VolumeBackupParams
		if err := decodeStrict(raw, &params); err != nil {
			return nil, invalidParams(err)
		}
		return s.volumeBackup(ctx, params)
	case "volumes.restore":
		if err := s.requireDocker(); err != nil {
			return nil, err
		}
		var params VolumeRestoreParams
		if err := decodeStrict(raw, &params); err != nil {
			return nil, invalidParams(err)
		}
	...
	case "secrets.action":
		...
		var params SecretsActionParams
		if err := decodeStrict(raw, &params); err != nil {
			return nil, invalidParams(err)
		}
		return s.secretsAction(ctx, params)
	case "system.action":
		if err := s.requireDocker(); err != nil {
			return nil, err
		}
